...
pub struct Settings {
    ...
    // Overlay transparency (false = opaque/OBS-friendly, true = transparent)
    pub overlay_stream_mode: bool,
}
    ...
    pub fn load() -> Result<Settings> {
        ...
        let result = conn.query_row(
            "SELECT poe_log_path, account_name, overlay_enabled, overlay_opacity, sound_enabled, overlay_x, overlay_y,
                    ...
                    overlay_show_breakpoints, overlay_hide_deaths, overlay_breakpoint_count, overlay_bg_opacity, overlay_accent_color,
            ...
            |row| {
                Ok(Settings {
                    ...
                    overlay_hide_deaths: row.get(13)?,
                    overlay_breakpoint_count: row.get(14)?,
                    overlay_bg_opacity: row.get(15)?,
                    overlay_accent_color: row.get(16)?,
                    overlay_always_on_top: row.get(17)?,
                    ...
                    hotkey_manual_split: row.get(22)?,
                    group_mode_enabled: row.get(23)?,
                    show_town_visits: row.get(24)?,
                    minimize_to_tray: row.get(25)?,
                    endgame_enabled: row.get(26)?,
                    overlay_stream_mode: row.get(27)?,
                })
            },
        );
        ...
        conn.execute(
            "INSERT INTO settings (id, poe_log_path, account_name, overlay_enabled, overlay_opacity, sound_enabled, overlay_x, overlay_y,
             ...
             ON CONFLICT(id) DO UPDATE SET
                ...
                overlay_hide_deaths = excluded.overlay_hide_deaths,
                ...
                overlay_stream_mode = excluded.overlay_stream_mode",
            params![
