...
function App() {
      ...
      try {
        ...
        if (settings) {
          loadSettings({
            ...
            overlayScale: (settings.overlay_scale || 'small') as 'small' | 'medium' | 'large',
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'mapping':
        return <Suspense fallback={<ViewLoader />}><MappingView /></Suspense>;
      case 'settings':
        return <SettingsView />;
      default:
        return <TimerView />;
  ...
  return (
    <div className="flex h-screen bg-[--color-poe-darker] noise-bg">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <footer className="flex-shrink-0 px-3 py-1 text-center border-t border-[--color-border]/50">
          <span className="text-[10px] text-[--color-text-muted]/40" title="Not affiliated with or endorsed by Grinding Gear Games">
